    ...
    for i in range(n):
        if len(fpr[i]) != len(tpr[i]):
            ...
            ax.plot(
                ...
                alpha=alpha,
                lw=lw,
                label='ROC %r (AUC=%0.2f;KS=%0.2f)' % (names[i], auc_score,
                                                       ks_score))
    # plot mean tpr line
    if n > 1:
...
def plotKS(y_pred, y_true, n, asc):
    ...
    plt.axhline(
        ksds.loc[ksds.ks.idxmax(), 'cumsum_good'],
        color='blue',
        linestyle='--')
    plt.axhline(
        ksds.loc[ksds.ks.idxmax(), 'cumsum_bad'], color='red', linestyle='--')
    plt.title(
        'KS=%s ' % np.round(ks_value, 4) + 'at Pop=%s' % np.round(ks_pop, 4),
        fontsize=15)
...
def plotter_rateVol(df,
    ...
    axe.annotate(
        'mean rate=%-0.2f%%(vol=%d)' % (rate_weighted * 100, sum(vol)),
